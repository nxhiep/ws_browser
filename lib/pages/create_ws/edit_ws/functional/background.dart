...
class BackgroundItem extends StackBoardItem {
  final String? imageUrl;
  final String? color;
  const BackgroundItem({ required Widget child, this.color, this.imageUrl }) : super(child: child);
}

class BackgroundFunctional extends StatefulWidget {
  final void Function(Color color) onSelectedColor;
  final void Function(String imageUrl) onSelectedImage;
  const BackgroundFunctional({ Key? key, required this.onSelectedColor, required this.onSelectedImage }) : super(key: key);

  @override
  State<BackgroundFunctional> createState() => _BackgroundFunctionalState();
}

class _BackgroundFunctionalState extends State<BackgroundFunctional> {

  @override
  void initState() {
    super.initState();
    WidgetsBinding.instance.addPostFrameCallback((_) { 
      Provider.of<BackgroundProvider>(context, listen: false).loadData();
    });
  }
